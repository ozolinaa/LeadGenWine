...
namespace LeadGen.Areas.Admin.Controllers
{
    public class TaxonomyController : AdminBaseController
    {
        ...
        public JsonResult GetTaxonomyTerms(int taxonomyID, string termSearch)
        {
            Taxonomy tax = Taxonomy.SelectFromDB(DBLGcon, TaxonomyID: taxonomyID).First();
            tax.LoadTerms(DBLGcon);
            var terms = tax.termList.Where(x=> x.name.ToLower().Contains(termSearch.ToLower()));
            return new JsonResult() { Data = terms, JsonRequestBehavior = JsonRequestBehavior.AllowGet };
        }

        [HttpPost]
        public JsonResult CreateTag(int taxonomyID, string tagName, string tagUrl = "")
        {
            Taxonomy tax = Taxonomy.SelectFromDB(DBLGcon, TaxonomyID: taxonomyID).First();
            tax.LoadTerms(DBLGcon);
            ...
            return new JsonResult() { Data = tag, JsonRequestBehavior = JsonRequestBehavior.AllowGet };
        }
